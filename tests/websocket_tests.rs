...
use uniclipboard::{
    ...
    web::WebServer,
    Config, WebSocketHandler, CONFIG,
};
...
fn setup_webserver(websocket_handler: Arc<WebSocketHandler>) -> WebServer {
...
async fn test_websocket_run() {
    ...
    let webserver = Arc::new(setup_webserver(websocket_handler));
            ...
            match client1.receive_raw().await {
                Ok(message) => {
                    ...
                    match serde_json::from_str(&message) {
                        Ok(WebSocketMessage::DeviceListSync(data)) => {
...
async fn test_websocket_subscribe() {
    ...
    let websocket_handler_clone = Arc::clone(&websocket_handler);
    let handle1 = tokio::spawn(async move {
        match websocket_handler_clone.subscribe().await {
            Ok(Some(message)) => {
